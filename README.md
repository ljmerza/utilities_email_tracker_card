...
<img src="card.png" alt="Utilities Email Tracker Card Screenshot" width="400"/>
...
- Optional **Title**, **Provider filter**, **Displayed fields**, and **Bill
  index** can be adjusted in the card editor. Leave the bill index blank to
  list every available bill.
...
details so you can scan every utility in one place. Use the displayed fields
settings to pick from Amount Due, Due Date, Account Number, and Billing Period.
The provider filter performs a flexible match against the bill attributes, so
you can enter a partial provider name (for example just `PSNC`) or sender
address to target the correct bill.
